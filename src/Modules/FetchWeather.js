...
class FetchWeather {
  ...
  async getWeather(city, DOMcallback, databaseCallback, resolve, reject){
    ...
    const { timezone } = city;
    const responseWeather = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&daily=weathercode,temperature_2m_max,temperature_2m_min,sunrise,sunset&hourly=temperature_2m,precipitation&current_weather=true&timezone=${timezone}`); 
